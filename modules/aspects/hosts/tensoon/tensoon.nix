  ...
  den.aspects.tensoon = {
    ...
    nixos =
      ...
      }:
      {
        ...
        # `pass-cli login` (its name is a fixed account hash), so the throwaway DB and the stored key
        # desync for good -> SQLCipher HMAC failure. The unit is WantedBy=default.target, so it fires
        # every boot before a shell exists; on this tmpfs-root host the session dir is freshly
        # restored each boot too. Gate ExecStart on the real session file appearing - a no-session
        # pass-cli.db has no session.json - and never invoke pass-cli in the guard itself.
        # StartLimitIntervalSec=0 so the guard failing for minutes on a not-yet-logged-in boot
        # doesn't trip start-limit-hit.
        home-manager.users.oscar.systemd.user.services.proton-pass-agent = {
          Service = {
            ExecStartPre = lib.getExe (
              pkgs.writeShellApplication {
                name = "proton-pass-agent-await-session";
                runtimeInputs = [ pkgs.coreutils ];

                text = ''
                  sess="''${XDG_DATA_HOME:-$HOME/.local/share}/proton-pass-cli/.session/session.json"
