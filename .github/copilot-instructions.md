...
- A daily workflow (`update-minecraft-mods.yml`) re-resolves harmony's Minecraft mods and opens a PR (mod-only bumps
  auto-merge; Minecraft upgrades are labelled `minecraft-game-version` and wait for review).
- **Update Minecraft mods**: `nix run .#update-minecraft-mods` (see "Working with Minecraft Mods" below)
...
### Working with Minecraft Mods

harmony's Fabric worlds (`modules/aspects/hosts/harmony/minecraft-servers.nix`) declare `mods` by Modrinth slug, with an
optional per-mod `channel` (`"release"` default, `"beta"`, `"alpha"`) and an optional per-world `gameVersion` pin.
`my.minecraft-servers` takes **both** the Fabric server package and the `mods` symlink from
`modules/aspects/hosts/harmony/minecraft-mods.lock.json`, never from `fabricServers.fabric` (latest) - so a
nix-minecraft bump can't move a world onto a Minecraft version its mods don't support (what crash-looped `vanilla` after
#896).

- The lock is generated by `nix run .#update-minecraft-mods` (`modules/aspects/my/minecraft-servers/update-mods.py`) -
  never edit it by hand. Evaluation fails with a "run the updater" message whenever a world's mod names, channels or pin
  disagree with the lock, so rerun it after any `mods`/`gameVersion` change.
- An unpinned world resolves to the newest Minecraft release that nix-minecraft packages AND every mod supports, and the
  updater refuses to downgrade a world rather than silently doing so.
- Every managed world runs nix-minecraft's own JDK for its version (`vanillaServers.<version>.java`), not nixpkgs'
  ambient `jre_headless`.
