...
- **`secrets/`**: Directory containing ragenix/agenix-rekey-encrypted secrets (`.age` files). Primitive secrets
  are encrypted to the YubiKey master identity. Do not expose plaintext. Rekeyed host secrets live in
  `modules/aspects/hosts/<hostname>/secrets/`.
...
2. **Secrets Management**: Secrets are managed via ragenix + agenix-rekey with a YubiKey master identity. Primitive
   secrets live in `secrets/*.age` (encrypted to YubiKey, `intermediary = true`). Template/composite secrets are
   generated with `agenix generate` and rekeyed per host with `agenix rekey -a`. Rekeyed outputs live in
   `modules/aspects/hosts/<hostname>/secrets/`. Never commit plaintext secrets or edit `.age` files directly.
   Use the `nixosSecrets` class for NixOS-only secrets (user passwords, etc.) to prevent them from being rekeyed
   to Darwin hosts. Each NixOS host that uses secrets must set `age.rekey.hostPubkey` in its host aspect.
...
- Secrets are managed via ragenix + agenix-rekey with a YubiKey master identity (age encryption). Primitive secrets
  are never rekeyed directly to hosts; template secrets are generated and then rekeyed per host.
...
- **`nixosSecrets` class**: Use in user/host aspects for NixOS-only secrets (e.g. hashed passwords). Forwarded only
  into `nixos.age.secrets` by `defaults.nix`, never to Darwin or Home Manager. Requires `age.rekey.hostPubkey` set
  on each NixOS host that uses it.
...
- **Primitive secret paths**: Use relative paths pointing to `secrets/`: `../../../secrets/filename.age` (adjust for
  depth). These are encrypted to the YubiKey (`intermediary = true`).
- **Rekeyed secret references**: Use `rekeyFile` in `nixos.age.secrets.<name>` to point at the primitive `.age` file.
  The rekeyed copy in `modules/aspects/hosts/<hostname>/secrets/` is auto-generated by `agenix rekey -a`.
...
**Creating or editing a primitive secret:**
...
1. Add an `age.secrets.<name>` entry with `rekeyFile` and `intermediary = true` in the relevant aspect.
2. Run `ragenix -e secrets/<name>.age` to encrypt the value.

**Adding a new template/generated secret:**

1. Define a `generator` block in the relevant `age.secrets.<name>` entry (see existing aspects for examples).
2. Run `agenix generate` to produce the template secret.

**Rekeying after adding a host or changing secrets:**

```bash
agenix rekey -a
git add modules/aspects/hosts/<hostname>/secrets/
```
